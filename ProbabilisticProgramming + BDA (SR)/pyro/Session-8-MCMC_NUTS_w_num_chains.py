...
from pyro.infer.autoguide import AutoMultivariateNormal, init_to_mean, init_to_value, AutoNormal, AutoLaplaceApproximation
...
def model_legsim(data, training=False):
    left = data["leg_left"]
    right = data["leg_right"]
    height =data["height"]   
    ...
    mu = pyro.deterministic("mu", a + bl*left + br*right)
    if training:
        with pyro.plate("N"):
...
if __name__ == "__main__":
    ...
    m9_1 = train_nuts(model_m9_1, data, 1000, 1000, num_chains) # I should have 4 cpu's, but pyro only recognizes 3...
    ...
    print(s.keys())
    print(s.keys(), s["sigma"].shape)
    for var in ("a", "b"):
        for i in (0, 1):
            s[f"{var}[{i}]"] = s[var][:,:,i]
        del s[var]
    print(len(s))
    f = traceplot(s, num_chains)
    plt.show()
    ...
    data = {"y": tt([-1., 1.])}
    m9_2 = train_nuts(model_m9_2, data, 1000, 1000, num_chains)
    print(m9_2.summary())
    ...
    trankplot(s, num_chains)
    plt.show()
    az.plot_rank(az.from_pyro(m9_2))
    plt.show()

    print(f"Starting M9_3 {num_chains}-chain")
    data = {"y": tt([-1., 1.])}
    m9_3 = train_nuts(model_m9_3, data, 1000, 1000, num_chains)
    print(m9_3.summary())
    ...
    plt.show()
    az.plot_rank(az.from_pyro(m9_3))
    plt.show()
    
    print(f"Starting M9_4 {num_chains}-chain")
    m9_4 = train_nuts(model_m9_4, data, 1000, 1000, num_chains)
    print(m9_4.summary())
    ...
    plt.show()
    az.plot_rank(az.from_pyro(m9_4))
    plt.show()

    print(f"Starting M9_5 {num_chains}-chain")
    m9_5 = train_nuts(model_m9_5, data, 2000, 2000, num_chains)
    print(m9_5.summary())
    ...
    plt.show()
    az.plot_rank(az.from_pyro(m9_5))
    plt.show()
    
    print(f"Starting M9_6 {num_chains}-chain")
    m9_6 = train_nuts(model_m9_6, data, 1000, 1000, num_chains)
    print(m9_6.summary())
    ...
    plt.show()
    az.plot_rank(az.from_pyro(m9_6))
    plt.show()
    '''
    num_chains =3
    print(f"Starting m_legsim {num_chains}-chain")
    N = 100
    ...
    m_legsim = train_nuts(model_legsim, data, 1000, 1000, num_chains)
    print(m_legsim.summary())
    traceplot(m_legsim.get_samples(group_by_chain=True), num_chains)
    plt.show()
    az.plot_trace(az.from_pyro(m_legsim))
    plt.show()
    trankplot(m_legsim.get_samples(group_by_chain=True), num_chains)
    plt.show()
    az.plot_rank(az.from_pyro(m_legsim))
    plt.show()
    '''